    ...
    public class Program
    {
        ...
        internal static void StartAgent(ShdrAgentConfiguration configuration, bool verboseLogging = false, int port = 0)
        {
            if (!_started && configuration != null)
            {
                ...
                var initializeDataItems = !_observationBuffer.Load();
                ...
                {
                    foreach (var adapter in _adapters) adapter.Start();
                }
                ...
                _server = new ShdrMTConnectHttpServer(configuration, _mtconnectAgent, null, port);
