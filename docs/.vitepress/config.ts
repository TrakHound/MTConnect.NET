 ...
 *
 * The site root is `docs/` (the existing top-level documentation
 * directory). Mermaid diagrams are enabled site-wide via
 * `vitepress-plugin-mermaid`, so every architecture / sequence /
 * class-relationship / state-machine / wire-flow diagram is authored
 * in Mermaid (no ASCII art, no external image renders for schematic
 * content).
 *
 * @remarks
