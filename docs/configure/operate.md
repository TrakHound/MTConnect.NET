...
- **Tail a specific module or processor**: `tail -F logs/modules.<module-id>-$(date +%F).log` (e.g. `logs/modules.http-server-$(date +%F).log`); same shape for processors via `logs/processors.<processor-id>-$(date +%F).log`.
