...
The shipped agent (`MTConnect.NET-Agent`) and adapter (`MTConnect.NET-Adapter`) accept the same command set. Each verb maps to a behaviour the underlying [`MTConnectAgentApplication`](/api/MTConnect.Applications.MTConnectAgentApplication) (or the adapter equivalent) implements.
