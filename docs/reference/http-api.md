...
This page lists every HTTP endpoint exposed by an MTConnect.NET agent. The list is generated by walking the source tree with Roslyn.
...
- **Admin / diagnostic** — none. No administrative endpoint is currently exposed. A search for `/admin`, `/diagnostics`, `/metrics`, or `/health` across the source tree returns no hits.
