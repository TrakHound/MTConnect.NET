...
//
// Usage: check-broken-links.mjs [<docs-root>] [--exclude=<glob-fragment>]
//   --exclude=api/   skip every file whose path contains "/api/" (faster
//                    local pass when the auto-generated reference tree
//                    is regenerated by docfx anyway).
...
const getMarkdownFiles = async (directory, excludeFragments) => {
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
            ...
            const nested = await getMarkdownFiles(fullPath, excludeFragments);
            markdownFiles.push(...nested);
        } else if (entry.isFile() && entry.name.endsWith('.md')) {
            if (excludeFragments.some((fragment) => fullPath.includes(fragment))) continue;
...
const candidatePaths = (basePath, relativePath, docsRoot) => {
    ...
    if (cleaned === '') {
        ...
        root = resolve(docsRoot, 'index');
    } else {
        const decoded = safeDecode(cleaned);
        root = decoded.startsWith('/') ? resolve(docsRoot, `.${decoded}`) : resolve(basePath, decoded);
    }
...
const parseArgs = (argv) => {
    ...
    for (const arg of argv) {
        if (arg.startsWith('--exclude=')) {
            const fragment = arg.slice('--exclude='.length);
            if (fragment) excludeFragments.push(fragment.replaceAll('/', sep));
        } else if (arg === '--help' || arg === '-h') {
            console.info('Usage: check-broken-links.mjs [<docs-root>] [--exclude=<path-fragment>]');
            process.exit(0);
        } else {
            positional.push(arg);
        }
    }
    return { root: positional[0] ?? '.', excludeFragments };
};

const checkLinks = async ({ root, excludeFragments }) => {
    const docsRoot = resolve(root);
    const isDir = await isDirectory(docsRoot);
    const files = isDir ? await getMarkdownFiles(docsRoot, excludeFragments) : [docsRoot];
