...
{
    /// <summary>
    /// Ceen request handler for the MTConnect input endpoint over HTTP
    /// PUT (PUT / and PUT /{deviceKey}). Accepts data-item observation
    /// input encoded as key=value pairs in the request body or query
    /// string, validates the device-relative scope, and forwards each
    /// pair to the configured ProcessFunction for ingestion into the
    /// agent. The response carries an HTTP status (200 on accept, 400
    /// on malformed input, 404 on unknown device) and an optional
    /// MTConnectError body when negotiated.
    /// </summary>
