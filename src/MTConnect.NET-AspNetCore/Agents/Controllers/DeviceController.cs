...
namespace MTConnect.Agents.Controllers
{
    ...
    public class DeviceController : ControllerBase
    {
        ...
        [HttpGet]
        [HttpGet("probe")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(DevicesDocument))]
            ...
            )
        {
            ...
            var response = await MTConnectHttpRequests.GetDeviceProbeRequest(_agent, deviceName, version, documentFormat, indent);
            ...
            )
        {
            ...
            var response = await MTConnectHttpRequests.GetDeviceCurrentRequest(_agent, deviceName, path, at, interval, version, documentFormat, indent);
            ...
            )
        {
            ...
            if (interval > 0)
            {
                try
                {
                    ...
                    var stream = new MTConnectHttpStream(_agent, deviceName, path, from, count, interval, heartbeat, documentFormat, indent);
            ...
            else
            {
                ...
                var response = await MTConnectHttpRequests.GetDeviceSampleRequest(_agent, deviceName, null, from, to, count, version, documentFormat, indent);
