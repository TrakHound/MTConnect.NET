            ...
            )
        {
            ...
            if (interval > 0)
            {
                try
                {
                    ...
                    var stream = new MTConnectHttpServerStream(_configuration, _agent, deviceKey, dataItemIds, 0, 0, interval, heartbeat, documentFormat, null, formatOptions);
            ...
            )
        {
            ...
            if (interval > 0)
            {
                try
                {
                    ...
                    var stream = new MTConnectHttpServerStream(_configuration, _agent, deviceKey, dataItemIds, from, count, interval, heartbeat, documentFormat, null, formatOptions);
                    stream.StreamStarted += (s, id) =>
                    {
