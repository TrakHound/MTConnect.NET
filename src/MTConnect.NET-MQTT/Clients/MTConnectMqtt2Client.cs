        ...
        private void ProcessCurrentDocument(IStreamsResponseDocument document)
        {
            ...
            if (document != null)
            {
                if (!document.Streams.IsNullOrEmpty())
                {
                    ...
                    {
                        long lastSequence;
                        lock (_lock) lastSequence = _deviceLastSequence.GetValueOrDefault(deviceStream.Uuid);
            ...
            if (document != null)
            {
                ...
                if (!document.Streams.IsNullOrEmpty())
                {
                    ...
                    if (deviceStream != null && deviceStream.Observations != null && deviceStream.Observations.Count() > 0)
                    {
                        ...
                        lock (_lock)
                        {
                            lastCurrentSequence = _deviceLastCurrentSequence.GetValueOrDefault(deviceStream.Uuid);
