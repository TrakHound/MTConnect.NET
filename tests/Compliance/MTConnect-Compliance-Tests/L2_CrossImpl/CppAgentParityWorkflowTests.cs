...
namespace MTConnect.Compliance.Tests.L2_CrossImpl
{
    ...
    // requests the same envelope from both, normalizes out the runtime-
    ...
    public class CppAgentParityWorkflowTests
    {
        ...
        private async Task CompareEnvelopes(string path, string expectedRootLocalName)
        {
            ...
            if (cppCanonical != netCanonical)
            {
                ...
                Assert.Fail(
                    $"{path}: cppagent and MTConnect.NET shapes diverge after whitelist normalization.\n{diff}");
            }
        }
        ...
        private static string NormalizeAttributeValue(string raw)
        {
            ...
            // default for bool.ToString()). Normalize both sides to
        ...
        {
            // Honor an env-var override so a CI runner can stage the
