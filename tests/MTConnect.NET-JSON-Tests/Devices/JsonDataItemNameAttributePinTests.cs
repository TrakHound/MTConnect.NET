...
{
    /// <summary>
    /// Metadata-level pin for the base JSON Probe DataItem `name` contract.
    ///
    ...
    /// consumers that probe the property metadata directly — or hand the DTO to a
    /// fresh `JsonSerializer` instance with no project options — must still see the
    /// property omitted when null. The `[JsonIgnore(Condition = WhenWritingNull)]`
    /// attribute pins that contract on the type itself so it travels with the DTO.
    ///
    /// Source authority:
