...
{
    /// <summary>
    /// Pins the JSON-cppagent Probe DataItem behavior: the `name` JSON property must be
    ...
    public class JsonDataItemEmptyNameOmissionTests
    {
        ...
        public void Constructor_with_explicit_Name_source_serializes_name_key()
        {
            ...
            Assert.That(doc.RootElement.TryGetProperty("name", out var nameElement), Is.True,
                "JSON-cppagent Probe DataItem must emit 'name' when source Name has a value");
            Assert.That(nameElement.GetString(), Is.EqualTo("temp"));
        }

        /// <summary>Pins the behaviour expressed by the test name: constructor with typed data item unset name does not serialize name key.</summary>
        [Test]
        public void Constructor_with_typed_DataItem_unset_Name_does_not_serialize_name_key()
        {
            // TemperatureDataItem default ctor sets Name = NameId ("temp"); a caller
            // that wants to clear it can assign an empty string. Confirm the wire
            // honors the cleared state.
